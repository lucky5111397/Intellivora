...
export const validateAnalysisResponse = (response) => {
  ...
  const interviewReadinessScore = parseNumber(
    response.interviewReadinessScore
  );
  ...
  const finalResponse = {
    ...
    interviewReadinessScore: Math.min(
      100,
      Math.max(0, interviewReadinessScore)
    ),
  ...
  requiredFields.forEach((field) => {
    if (!Object.prototype.hasOwnProperty.call(response, field)) {
      throw new Error(
        `AI analysis response is missing required field: ${field}`
      );
    }
  });
...
export const analyzeResumeText = async ({
  ...
  if (!isString(extractedText)) {
    const error = new Error(
      "Extracted text is required and must be a non-empty string."
    );
    error.status = 400;
    throw error;
  }
  ...
  if (!isString(targetRole)) {
    const error = new Error(
      "targetRole is required and must be a non-empty string."
    );
    error.status = 400;
    throw error;
  }
  ...
  if (!isString(experienceLevel)) {
    const error = new Error(
      "experienceLevel is required and must be a non-empty string."
    );
    error.status = 400;
    throw error;
  }
  ...
  const messages = [
    {
      role: "system",
      content:
        "You are a professional resume analyst and career coach. Return valid JSON only.",
    },
    {
      role: "user",
      content: buildPrompt({
